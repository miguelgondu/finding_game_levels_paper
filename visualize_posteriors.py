...
def plot_all_updates(
    folder_name, agent_name, feat_1, feat_2, pattern="*", comment=None
):
    all_partitions = {
    ...
    association = {key: i for i, key in enumerate(keys)}

    keys_partition = list(partition.keys())
    keys_partition.sort()

    update_path = glob.glob(
        f"./zelda_experiments/updates/{folder_name}/{agent_name}/{pattern}_*.json"
    )
    ...
    for path in update_path:
        ...
        proj_point = (
            tested_centroid[association[keys_partition[0]]],
            tested_centroid[association[keys_partition[1]]],
        )
        ax1.plot(proj_point[0], proj_point[1], "ok")

        ax1.set_title(f"Mean performance \n {agent_name} using prior {folder_name}")
        # plt.show()
        path_to_fig = path.replace(
            f"/updates/{folder_name}/{agent_name}/", "/posteriors/"
        )
        ...
        if comment:
            ...
            _list[
                -1
            ] = f"posterior_{folder_name}_to_{agent_name}_{feat_1}_{feat_2}_{iteration}.jpg"
        ...
        save_level_from_array(
            level,
            f"./zelda_experiments/posterior_levels/{level_name}",
            title=f"Centroid: {tested_centroid}",
        )
