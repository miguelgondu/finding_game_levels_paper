...
saves a table with them. (Table 1)
"""
import click
import json
import glob
...
def compute_amounts(filepath):
    ...
    amounts = {diff: 0 for diff in difficulties}
...
totals = [sum(df.loc[name]) for name in df.index]
df["totals"] = totals

print(df)
with open(
    "./zelda_experiments/final_tables/levels_segmented_difficulty.csv", "w"
) as fp:
    fp.write(df.to_csv())

with open(
    "./zelda_experiments/final_tables/levels_segmented_difficulty_tex.txt", "w"
) as fp:
