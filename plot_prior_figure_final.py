...
partition_3 = {"space coverage": space_coverage, "reachability": reachability}

partition_2 = {"leniency": leniency, "reachability": reachability}

partition_1 = {"space coverage": space_coverage, "leniency": leniency}

all_partitions = {**partition_1, **partition_2}
...
def plot_prior(generation, prior_name, size=250):
    ...
    for partition in [partition_1, partition_2, partition_3]:
        ...
        plot_mean_winrate(
            ax, generation, partition, size=size, plot_labels=False, plot_colorbar=False
        )
        keys = list(partition.keys())
        keys.sort()
        plt.savefig(
            f"{path_to_prior}/{keys[0]}_{keys[1]}.jpg", dpi=150, bbox_inches="tight"
        )
        plt.close()
